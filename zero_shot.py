...
def prf1(pred, gold):
    """Lists of strings → (precision, recall, f1)."""
...
for rk, p in lora_paths.items():
    ...
    tok = AutoTokenizer.from_pretrained(p["tok"])
...
def build_prompt(sample):
    ...
    return (
        f"### Task:\nExtract the exact name or names of {header} from the input "
        f"text and output them in a list.\n\n"
        f"### Definition:\n{definition}\n\n"
        f"### Input Text:\n{sample['context']}\n\n"
        f"### Output:"
    )
...
for rk in lora_paths:
    ...
    for sample in test_ds:
        ...
        gen_answer = full[len(prompt):].strip()

        pred = [s.strip() for s in re.split(r"[,\n]+", gen_answer) if s.strip()]
    ...
    n_docs = len(test_ds)
    overall_p = sum(sum(v["p"]) for v in cat_scores.values()) / n_docs
    overall_r = sum(sum(v["r"]) for v in cat_scores.values()) / n_docs
    overall_f = sum(sum(v["f"]) for v in cat_scores.values()) / n_docs

    GLOBAL["rank_name"].append(rk)
    GLOBAL["precision"].append(overall_p)
    GLOBAL["recall"].append(overall_r)
    GLOBAL["f1"].append(overall_f)
...
pd.DataFrame(GLOBAL).to_csv(f"{out_dir}/metrics_overall.csv", index=False)
pd.DataFrame(BY_CAT).to_csv(f"{out_dir}/metrics_by_entity.csv", index=False)

print("✓ Wrote metrics_overall.csv and metrics_by_entity.csv")
